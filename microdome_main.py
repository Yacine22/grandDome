...
class user_interface:
    ...
    def __init__(self): 
        ...
        self.dome_type = "MICRO DOME" ## Change Here for Dome Model
        self.aq_type = "RAPIDE"
...
### ----------------------------------------   Menu Reglages ---------------------------------------------------------------------
    ...
    def menu_reglages(self):
        self.reglage_interface = Toplevel(self.interface)
    ...
    def copy_to_usb_(self):
        ...
        if len(folders_in_media) == 0:
            ...
            if round((free/2**30), 2) > 1.0 and project_existance == False :                
                t = threading.Thread(target=ext.blinkingRGB, args=("task",))
                t.start()
    ...
    def _aquisition_(self, image_nb):
        ...
        self.capture_wind_aq.update()
        os.system("sudo chmod 777 /home/pi/grandDome/images/rti/IMAGES_85/rti/*")
...
class others:
    ...
    def __init__(self):
        self.envi_windO = Tk()
        self.envi_windO.attributes('-fullscreen', True)
        self.envi_windO.title('Autres')
        self.envi_windO.configure(bg="#212121")
        #self.envi_windO.geometry("800x480")
        
        
        self.frame_exit = Frame(self.envi_windO, bg="#212121")
        self.frame = Frame(self.envi_windO, bg="#212121")
        keypad_frame = Frame(self.envi_windO, bg="#212121")
        ...
        self.retour____ = ImageTk.PhotoImage(master=self.envi_windO, image=self.retour___icone)
        self.button_exit = Button(self.frame_exit, text="Sortir", bg='#212121', command=self.envi_windO.destroy)
        ...
        self.__logo__au = ImageTk.PhotoImage(master=self.envi_windO, image=self._logo_mercurio_au)
    ...
    def set_text_W(self, text):
        ...
        self.mot_de_passeWifi.insert("insert", text)
            
    def delete_text_W(self):
        self.mot_de_passeWifi.delete(len(self.mot_de_passeWifi.get())-1, END)
        
    def select_text_wifi(self, text):
        global wifi_ssid
        self.selection = text.widget.curselection()
        self.index = self.selection[0]
        self.value = text.widget.get(self.index)
    ...
    def wifi_conf(self):
        ...
        fread = open("/home/pi/grandDome/wpa_supplicant.conf", "r+")
        f = open("/home/pi/grandDome/wpa_supplicant.conf", "a+")
        wifi_data = "\n"+"network={\n\t"+"ssid="+wifi_ssid+"\n\t"+"psk="+self.pswrd+"\n\t"+"proto=RSN"+"\n\t"+"key_mgmt=WPA-PSK"+"\n\t"+"pairwise=CCMP\n\t"+"auth_alg=OPEN"+"}"
        if wifi_data in fread.read():
            print("SSID Exists")
        else:
            f.write(wifi_data)
            f.close()
        os.system("sudo cp wpa_supplicant.conf /etc/wpa_supplicant/wpa_supplicant.conf")
        os.system("wpa_passphrase "+str(wifi_ssid)+" "+str(self.pswrd)+" | sudo tee /etc/wpa_supplicant.conf")
        subprocess.call(["sudo", "wpa_supplicant", "-B", "-c", "/etc/wpa_supplicant.conf", "-i", "wlan0"])
    ...
    def connection_state(self):
        ...
        #self.envi_wind.geometry('800x480')
        self.wifi_wind_con.configure(bg="#212121")
        
        self.icone_deRetourCnx = Image.open(icons_path_+"IconeRetour.png").resize((65, 65))
    ...
    def chclavier2(self):
        ...
        self.chclavier['command'] = self.chclavierC
        
        for car, grid_value in settings.clavier2().items():
            
            if grid_value[0] == 5:
                self.button_kb = Button(keypad_frame, text=str(car), bg='#424035', fg='#FFF3AE', activebackground ='green', bd=0, font=("Roboto Mono", 15 * -1, "bold"), width=5, height=2,
                            borderwidth=0, state=NORMAL, relief='flat', command=lambda x=car: self.set_text_W(x))
                self.button_kb.grid(row=grid_value[0]-1, column=grid_value[1], padx=1, pady=2, sticky='news')
            
            if grid_value[0] == 6:
                self.button_kb = Button(keypad_frame, text=str(car), bg='#424035', fg='#FFF3AE', bd=5, font=("Roboto Mono", 15 * -1, "bold"), height=2,
                            borderwidth=0, state=NORMAL, command=lambda x=car: self.set_text_W(x))
                self.button_kb.grid(row=grid_value[0]-1, column=grid_value[1], pady=2, sticky='news')
        ...
        self.button_del = Button(keypad_frame, text='<', bg='#494949', fg='#FFFFFF', activebackground ='gray', font=('helvetica', 14, 'bold'), height=2,
                            borderwidth=0, state=NORMAL, command=self.delete_text_W)       
        self.button_del.grid(row=7, column=11, pady=2, sticky='news')
        keypad_frame.grid(row=0, column=0, sticky='s')
    ...
    def chclavierC(self):
        ...
        for car, grid_value in settings.clavierC().items():
        ...
        self.button_del = Button(keypad_frame, text='<', bg='#494949', fg='#FFF3AE', activebackground ='gray', font=('helvetica', 14, 'bold'), height=2,
                            borderwidth=0, state=NORMAL, command=self.delete_text_W)       
        self.button_del.grid(row=7, column=11, pady=2, sticky='news')
        keypad_frame.grid(row=0, column=0, sticky='s')
    
    def chclavierMaj(self):
        self.wifi_wind.update()
        self.chclavier["text"] = "abc"
        keypad_frame = Frame(self.wifi_wind, bg='#212121', relief='groove')
        
        self.chclavier['command'] = self.chclavier2
        
        for car, grid_value in settings.clavier().items():
...
class _camera_folder_:
    ...
    def __init__(self):
        self.envi_wind_c = Tk()
        self.envi_wind_c.attributes('-fullscreen', True)
        self.envi_wind_c.title('environment')
        #self.envi_wind.geometry('800x480')
        self.envi_wind_c.configure(bg="#212121")
        
        self.frame = Frame(self.envi_wind_c, bg="#212121")
...
if __name__ == '__main__':
    ...
    ### Camera options
    try:
        camera_folder = settings.queryCameras()
    except:
